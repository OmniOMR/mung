...
@dataclass
class DurableBeam(SceneObject):
    begin: Subevent
    end: Optional[Subevent] = None
    continue_: Optional[list[Subevent]] = None
    all_subevents: list[Subevent] = field(init=False, repr=False)
    _beam_value_mapping: dict[Subevent, BeamValueToken] | None = field(init=False, repr=False, default=None)

    def __post_init__(self):
        # assert types
        assert isinstance(self.begin, Subevent)
        assert self.end is None or isinstance(self.end, Subevent)
        assert self.continue_ is None or all(isinstance(x, Subevent) for x in self.continue_)
    ...
    def _check_onsets(self) -> None:
        if not isinstance(self.begin, Subevent):
            return
        onsets = [s.in_measure_fractional_onset for s in self.all_subevents]
    ...
    def _collect(self) -> list[Subevent]:
        output: list[Subevent] = [self.begin]
    ...
    def __len__(self) -> int:
        return len(self.all_subevents)
    
    def is_begin(self, subevent: Subevent) -> bool:
        return self.begin == subevent
    
    def is_end(self, subevent: Subevent) -> bool:
        if self.end is None:
            return False
        
        return self.end == subevent
    ...
    def _compute_beam_values(self) -> dict[Subevent, BeamValueToken]:
        ...
        if self.is_hook:
            ...
            begins = 0
            ends = 0
            for b in self.begin.beams:
                if not b.is_hook:
                    if b.is_begin(self.begin):
                        begins += 1
                    elif b.is_end(self.begin):
                        ends += 1
            ...
            if begins > ends:
                mapping[self.begin] = BeamValueToken.FORWARD_HOOK
            elif ends > begins:
                mapping[self.begin] = BeamValueToken.BACKWARD_HOOK
            else:
    ...
    def is_hook(self) -> bool:
        return len(self.all_subevents) == 1

    def beam_value(self, durable: Durable) -> BeamValueToken:
        if self._beam_value_mapping is None:
            self._beam_value_mapping = self._compute_beam_values()
        
        value = self._beam_value_mapping.get(durable.subevent)
        if value is None:
